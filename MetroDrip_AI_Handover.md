...
**Version 1.2 — amended post-implementation to record the two-level category taxonomy** (FR-21, NFR-7/8, D-07 to D-09).
...
├── config/admin.py     # branded AdminSite (v1.2)
├── apps/
...
| D-09 | **Added v1.2:** Django Admin is branded via a custom `AdminSite` — login page headed "Administrator Login", every other admin page "MetroDrip Administration". Installed by replacing `django.contrib.admin` with `config.admin.MetroDripAdminConfig` in `INSTALLED_APPS`, so existing `admin.site.register(...)` calls are untouched |
