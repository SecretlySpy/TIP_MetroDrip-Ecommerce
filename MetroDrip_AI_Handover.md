...
**Version 1.4 — amended post-implementation to record curated collection categories and mass seeding.**
Version 1.3 amended v1.2 to record the administrator / merchant console separation (FR-22, NFR-9/10, D-10 to D-12).
...
> **v1.4 amendment note.** Everything marked *v1.4* documents work already built and merged. §4 gains Curated Collections (New Arrivals, Best-Sellers, On-Sale, Pre-Order) as root categories. Rationale lives in `DECISIONS.md` (ADR-C-005). Mass generation scripts are documented in `AI Documentation Notes.md`. **Nothing in §3 Hard Invariants or §12 Out of Scope changed.**
>
...
**Curated Collections (amended v1.4 — see ADR-C-005).** In addition to structural apparel categories (Tops, Bottoms), the catalog supports collection-based root categories (e.g., "New Arrivals", "Best-Sellers", "On-Sale", "Pre-Order"). Products are assigned directly to these root collections without requiring a gendered subcategory. Products in the "On-Sale" collection feature discounted pricing logic applied statically at creation time (no dynamic discount math).
