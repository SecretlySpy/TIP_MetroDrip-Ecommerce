...
SUB_CATEGORIES = {
    ...
    "T-Shirts": [
        "Metro Core Box Tee",
        "Drip District Graphic Tee",
        "EDSA Nights Oversized Tee",
        "Manila Skyline Pocket Tee",
        "Barangay Heavyweight Tee",
        "Tribal",
    ][:5],  # Assignment specifies 5 each, but listed 6. We keep 5.
    "Hoodies & Sweatshirts": [
        "Skyline Pullover Hoodie",
        "Drip Zip-Up Hoodie",
        "Metro Fleece Crewneck",
        "Midnight Commute Oversized Hoodie",
        "Monsoon Tech Hoodie",
    ],
    "Polos & Button-Ups": [
        "Knit Polo from Kalye",
        "Weekend Cuban Shirt",
        "Metro Corduroy Overshirt",
        "Work Shirt from Drip",
        "Plaid Flannel Button-Up",
    ],
    # Bottoms
    "Denim": [
        "Metro Straight Cut Jeans",
        "Drip Baggy Denims",
        "Faded District Slim Jeans",
        "Carpenter Denim Pants",
        "Acid Wash Wide Leg Jeans",
    ],
    "Shorts": [
        "Kalye Cargo Shorts",
        "Metro Mesh Shorts",
        "Drip Denim Shorts",
        "Boardwalk Nylon Shorts",
        "Terry Lounge Shorts",
    ],
    "Trousers & Joggers": [
        # The PDF had a typo here ("Metro Snapback"). Fixing to actual trousers.
        "Metro Track Joggers",
        "Drip Utility Trousers",
        "Skyline Cargo Pants",
        "Monsoon Waterproof Joggers",
        "Corduroy Chino Pants",
    ],
...
def run_seed():
    ...
    for subcat_name, products in SUB_CATEGORIES.items():
        ...
        category, _ = Category.objects.get_or_create(
            name=subcat_name, defaults={"slug": slugify(subcat_name)}
        )
            ...
            product, created = Product.objects.get_or_create(
                ...
                defaults={
                    ...
                    "base_price": random.choice(
                        range(base_price_range[0], base_price_range[1], 10000)
                    ),
                    "is_active": True,
                },
            )
